...
// ─── Tipos ────────────────────────────────────────────────────────────────────
interface Props {
  navigation: StackNavigationProp<RootStackParamList, 'BulkFormulario'>;
  route: RouteProp<RootStackParamList, 'BulkFormulario'>;
}
...
// ─── Pantalla ─────────────────────────────────────────────────────────────────
export default function BulkFormularioEntregaScreen({ navigation, route }: Props) {
  const { estado, subestado: subestadoInicial, pedidos } = route.params;
  ...
  const handleSiguiente = () => {
    ...
    if (paso < totalPasos) {
      ...
      navigation.navigate('BulkEntrega', {
        pedidos,
    ...
    <TextInput
      ...
      style={{
        ...
        ...(multiline ? {
          ...
          borderBottomWidth: 2,
          borderBottomColor: inputFocused || value ? colors.primary : '#E5E7EB',
        }),
      }}
    />
  );
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.primary }} edges={['top']}>
      ...
      <View style={{
        ...
        <Text style={{ fontSize: 18, fontWeight: 'bold', color: colors.text, marginLeft: 4, flex: 1 }}>
          Formulario
        </Text>
        {/* Tag "Gestión masiva" en lugar de lista de pedidos */}
        <View style={{
          ...
          {Array.from({ length: totalPasos }).map((_, i) => (
            <View
              ...
              style={{
                ...
                height: 3,
                borderRadius: 2,
                backgroundColor: i < paso ? '#FFFFFF' : 'rgba(255,255,255,0.3)',
              }}
            />
          ))}
        </View>
      ...
      >
        ...
        <View style={{
          ...
          >
            <Text style={btnTextStyle(puedeAvanzar)}>
              {paso === totalPasos ? 'Confirmar' : 'Siguiente'}
            </Text>
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
